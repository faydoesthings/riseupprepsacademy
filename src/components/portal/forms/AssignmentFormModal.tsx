        ...
        <div className="modal-backdrop animate-fade-in">
          <div className="modal-panel max-w-md mx-4">
            ...
            <form onSubmit={onSubmit} className="modal-body space-y-5">
              ...
              <div>
                <label className="form-label-caps">Due date <span className="text-[#F78C1F]">*</span></label>
                <input type="date" name="dueDate" required className="form-input" />
              </div>
              <div>
                <label className="form-label-caps flex items-center gap-2">
                  <LinkIcon className="w-4 h-4" /> Resource link (optional)
                </label>
                <input type="url" name="fileUrl" placeholder="https://" className="form-input" />
              </div>
              <div className="modal-footer !px-0 !pb-0 !border-0">
                ...
                <button type="submit" disabled={isSubmitting} className="btn btn-primary flex-1 min-h-[44px] disabled:opacity-70">
                  {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : "Assign"}
                </button>
              </div>
            </form>
          </div>
